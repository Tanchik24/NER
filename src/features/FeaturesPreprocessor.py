...
class FeaturesPreprocessor:
    def __init__(self, tokenizer):
        ...
        self.pad_label_id: int = int(os.getenv('PAD_LABEL_ID'))
    ...
    def __cut_length(self, tokens: List[str], entity_labels_ids: List[int]) -> Tuple[List[str], List[int]]:
    ...
    def __add_sep_token(self, tokens: List[str], entity_labels_ids: List[int]) -> Tuple[List[str], List[int], List[int]]:
        tokens += [self.sep_token]
        entity_labels_ids += [self.pad_label_id]
        token_type_ids = [self.sequence_a_segment_id] * len(tokens)
        return tokens, entity_labels_ids, token_type_ids

    def __add_cls_token(self, tokens: List[str], entity_labels_ids: List[int], token_type_ids: List[int]) -> Tuple[
        List[str], List[int], List[int]]:
        tokens = [self.cls_token] + tokens
        entity_labels_ids = [self.pad_label_id] + entity_labels_ids
        token_type_ids = [self.cls_token_segment_id] + token_type_ids
        return tokens, entity_labels_ids, token_type_ids

    def __add_attention_mask(self, input_ids: List[int], token_type_ids: List[int], entity_labels_ids: List[int]):
        ...
        entity_labels_ids = entity_labels_ids + ([self.pad_label_id] * padding_length)
    ...
    def get_feautures(self, texts: List[List[str]], entities_list: List[List[int]], stage: str) -> List[Features]:
        ...
        for index, (text, entities) in enumerate(zip(texts, entities_list)):
            ...
            for word, entity in zip(text, entities):
                ...
                entity_label_id = [int(entity)] + [self.pad_label_id] * (len(word_tokens) - 1)
                entity_labels_ids.extend(entity_label_id)

            tokens, entity_labels_ids = self.__cut_length(tokens, entity_labels_ids)
            tokens, entity_labels_ids, token_type_ids = self.__add_sep_token(tokens, entity_labels_ids)
            tokens, entity_labels_ids, token_type_ids = self.__add_cls_token(tokens, entity_labels_ids, token_type_ids)
            input_ids = self.tokenizer.convert_tokens_to_ids(tokens)
            input_ids, attention_mask, token_type_ids, entity_labels_ids = self.__add_attention_mask(input_ids,
                                                                                                     token_type_ids,
                                                                                                     entity_labels_ids)
